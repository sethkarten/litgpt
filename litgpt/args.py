...
class TrainArgs:
    ...
    def gradient_accumulation_iters(self, world_size: int) -> int:
        """Number of iterations between gradient synchronizations"""
        gradient_accumulation_iters = self.batch_size(world_size) // self.micro_batch_size
        assert gradient_accumulation_iters > 0
        return gradient_accumulation_iters

    def batch_size(self, world_size: int) -> int:
        """Number of samples between optimizer steps per data-parallel rank"""
        batch_size = self.global_batch_size // world_size
        assert batch_size > 0
        return batch_size

    def warmup_iters(self, world_size: int, max_iters: int, train_dataloader) -> int:
        ...
        if self.lr_warmup_steps:
            return min(max_iters, self.lr_warmup_steps * self.gradient_accumulation_iters(world_size))
        return 0
