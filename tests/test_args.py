...
def test_compute_warmup_iters():
    ...
    assert train.warmup_iters(world_size=1, max_iters=1000, train_dataloader=range(10)) == 0
    ...
    assert train.warmup_iters(world_size=1, max_iters=1000, train_dataloader=range(10)) == 100
    # lr_warmup_steps multiplied by accumulation factor
    train.global_batch_size = 4
    assert train.warmup_iters(world_size=1, max_iters=1000, train_dataloader=range(10)) == 400
    assert train.warmup_iters(world_size=2, max_iters=1000, train_dataloader=range(10)) == 200
    # lr_warmup_steps truncated by max iters
    assert train.warmup_iters(world_size=1, max_iters=120, train_dataloader=range(10)) == 120

    # lr_warmup_fraction
    train = TrainArgs(global_batch_size=1, micro_batch_size=1, lr_warmup_steps=0, lr_warmup_fraction=0.3)
    assert train.warmup_iters(world_size=1, max_iters=1000, train_dataloader=range(100)) == 30
    # lr_warmup_fraction truncated by max iters
    assert train.warmup_iters(world_size=1, max_iters=20, train_dataloader=range(100)) == 20
    # lr_warmup_fraction rounds up
    assert train.warmup_iters(world_size=1, max_iters=1000, train_dataloader=range(5)) == 2
